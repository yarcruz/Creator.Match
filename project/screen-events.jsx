// screen-events.jsx — browse events (creator-facing) + event detail
...
function EventsExplore({ go }) {
  const [need, setNeed] = useStateEv("all");
  const [city, setCity] = useStateEv("all");
  const [pay, setPay] = useStateEv("all");
  const [mode, setMode] = useStateEv("all");
...
function EventDetail({ id, go, onPropose }) {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 40, alignItems: "start" }} className="profile-grid">
        <div>
          ...
          <div style={{ display: "flex", gap: 28, marginTop: 26, flexWrap: "wrap" }}>
            {[["Fecha", `${e.date}${e.time ? ` · ${e.time}` : ""}`], ["Presupuesto", evFree(e) && !evPaid(e) ? "Sin pago" : e.budget], ["Postulados", `${e.applicants}`]].map(([k, v]) => (
              <div key={k}>
...
function ProposalModal({ eventId, onClose, onSubmit }) {
  const e = eventById(eventId) || EVENTS[0];
  const [amount, setAmount] = useStateEv("");
  const [includes, setIncludes] = useStateEv(e.needs ? [...e.needs] : []);
  const [mode, setMode] = useStateEv(e.mode === "ambas" ? "colab" : (e.mode || "colab"));
  const [note, setNote] = useStateEv("");
  const [sent, setSent] = useStateEv(false);
  ...
  const submit = () => {
    ...
    const composed = `${incLabels ? "Incluye: " + incLabels : ""}${incLabels && note ? " · " : ""}${note}`.trim()
      || "Propuesta enviada.";
    setSent(true);
...
Object.assign(window, { EventsExplore, EventDetail, EventCard, ProposalModal });
