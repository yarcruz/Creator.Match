// screen-pricing.jsx — "Planes" (premium). Hidden unless monetization is enabled.
...
function PlanCard({ plan, year, pro, onPick }) {
  ...
  return (
    <div style={{
      ...
      <Button full variant={pro ? "primary" : "ghost"} size="lg" style={{ marginTop: 20 }}
        onClick={onPick}>{price === 0 ? "Empezar gratis" : "Hazte Pro"}</Button>
      <div style={{ display: "flex", flexDirection: "column", gap: 11, marginTop: 24 }}>
        {plan.features.map(([f, on], i) => (
          <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start",
            ...
            <span style={{ flexShrink: 0, marginTop: 1, fontWeight: 700,
              color: on ? "var(--accent)" : "var(--line)" }}>{on ? "✓" : "—"}</span>
            <span style={{ textDecoration: on ? "none" : "none", opacity: on ? 1 : 0.7 }}>{f}</span>
          </div>
        ))}
      </div>
    </div>
...
function Pricing({ go }) {
  const [role, setRole] = useStatePr("creador");
  const [year, setYear] = useStatePr(false);
    ...
    <div className="fade-in" style={{ maxWidth: 1080, margin: "0 auto", padding: "48px clamp(20px,4vw,40px) 80px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 34, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13.5 }}>
          ...
          <button onClick={() => setYear(!year)} style={{ width: 44, height: 26, borderRadius: 99, padding: 3,
            ...
            <span style={{ display: "block", width: 20, height: 20, borderRadius: 99, background: "var(--panel)",
              transform: year ? "translateX(18px)" : "none", transition: "transform 0.2s ease", boxShadow: "var(--shadow-sm)" }} />
          </button>
