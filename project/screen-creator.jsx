// screen-creator.jsx — multi-step "register as creator" flow
// Fields mirror the real CollabLink-Registro de Creadores form:
// Nombre, Correo, Teléfono, Social Links, Gear (equipo), Event Preferences, Message to Producer.
...
function CreatorFlow({ go, onPublish }) {
  ...
  // modality as one choice of three: Colaboración / Exclusivo / Ambas
  const MODE_CHOICES = [
    ...
    <FlowShell steps={CREATOR_STEPS} current={0} kicker="Registro de creador" title="Empecemos por tus datos"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Field label="Teléfono">
          <TextInput value={data.phone} onChange={v => set("phone", v)} placeholder="+57 300 000 0000" />
        </Field>
      </div>
  ...
  // 2 — Portafolio (proyectos como enlaces de nube)
  if (step === 2) return (
    <FlowShell steps={CREATOR_STEPS} current={2} kicker="Portafolio" title="Enlaza tu mejor trabajo"
      ...
      <Field label="Piezas de portafolio" hint="Sube fotos o clips, o pega el enlace de UNA pieza (un video de YouTube/Vimeo, un post). Cada pieza cuenta por separado.">
        <PortfolioBuilder value={data.projects} onChange={v => set("projects", v)} />
      </Field>
  ...
  // 3 — Preferencias (Event Preferences + tarifa + agenda + mensaje al productor)
  if (step === 3) return (
      ...
      {data.payPref !== "sinpago" && (
        <Field label="Tarifa base" hint={data.payPref === "todos" ? "Para los eventos remunerados." : undefined}>
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <select value={data.rateUnit} onChange={e => set("rateUnit", e.target.value)} style={{ ...selStyle, borderRadius: "var(--r)" }}>
...
function UploadTile({ primary }) {
  const [h, setH] = useStateC(false);
  return (
    <button onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      className="ph" style={{ height: 96, borderRadius: 12, border: `1.5px dashed ${h ? "var(--accent)" : "var(--line)"}`,
        background: h ? "var(--accent-wash)" : undefined, cursor: "pointer", flexDirection: "column", gap: 4 }}>
      <span style={{ fontSize: 20, color: h ? "var(--accent)" : "var(--ink-faint)", background: "none", border: "none", padding: 0 }}>＋</span>
      {primary && <span>arrastra o sube</span>}
    </button>
  );
}

function CreatorPreviewCard({ data, large }) {
  const hue = 264;
...
function ProjectLinks({ value, onChange }) {
  ...
  const add = () => {
    const u = url.trim();
    if (!u) return;
    const t = title.trim() || (linkSource(u).label + " · proyecto");
    onChange([...list, { title: t, url: u }]);
    setTitle(""); setUrl("");
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 150px" }}><TextInput value={title} onChange={setTitle} placeholder="Título del proyecto" /></div>
        <div style={{ flex: "2 1 220px" }}><TextInput value={url} onChange={setUrl} placeholder="https://drive.google.com/…" /></div>
        <Button variant="soft" onClick={add}>Añadir</Button>
      </div>
      {list.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
          {list.map((p, i) => {
            const s = linkSource(p.url);
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px",
                borderRadius: "var(--r)", background: "var(--canvas)", border: "1px solid var(--line)" }}>
                <span style={{ width: 30, height: 30, borderRadius: 8, display: "grid", placeItems: "center",
                  background: "var(--accent-wash)", color: "var(--accent-ink)", fontSize: 14, flexShrink: 0 }}>{s.icon}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: 14, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.title}</div>
                  <div style={{ fontSize: 12, color: "var(--ink-faint)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.label} · {p.url}</div>
                </div>
                <button onClick={() => remove(i)} title="Quitar" style={{ color: "var(--ink-faint)", fontSize: 20, padding: "0 4px", lineHeight: 1 }}>×</button>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div>
      ...
      {list.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px,1fr))", gap: 10, marginBottom: 14 }}>
          {list.map((p, i) => {
            ...
            const over = window.__monetize && i >= FREE_LIMIT;
            return (
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
        <div style={{ flex: "1 1 150px" }}><TextInput value={title} onChange={setTitle} placeholder="Título (opcional)" /></div>
        <div style={{ flex: "2 1 220px" }}><TextInput value={url} onChange={v => { setUrl(v); setErr(""); }} placeholder="https://youtu.be/… o vimeo.com/…" /></div>
...
Object.assign(window, { CreatorFlow, FlowShell, CreatorPreviewCard, UploadTile, ProjectLinks, PortfolioBuilder });
