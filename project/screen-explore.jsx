// screen-explore.jsx — browse creators + creator profile
...
/* ---------- Creator card (3 visual styles via cardStyle tweak) ---------- */
function CreatorCard({ c, onClick, cardStyle = "bordered" }) {
  if (cardStyle === "row") {
    return (
    ...
    <Card hover onClick={onClick} style={{ overflow: "hidden", display: "flex", flexDirection: "column" }}>
      {imageForward && (
        <div className="ph" style={{ height: 168, position: "relative" }}>
          ...
          {window.__monetize && c.top && <div style={{ position: "absolute", top: 12, right: 12 }}><Badge tone="accent">Pro</Badge></div>}
        </div>
      )}
...
function Explore({ go, t, highlight }) {
  const [cat, setCat] = useStateE("all");
  const [city, setCity] = useStateE("all");
  const [mode, setMode] = useStateE("all");
  const [tier, setTier] = useStateE("all");
  const [pay, setPay] = useStateE("all");
  const [sort, setSort] = useStateE("relevancia");
    ...
    <div className="fade-in" style={{ maxWidth: "var(--maxw)", margin: "0 auto", padding: "40px clamp(20px,4vw,40px) 80px" }}>
      ...
      {t.cardStyle === "row" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {list.map(c => <CreatorCard key={c.id} c={c} cardStyle="row" onClick={() => go("profile", c.id)} />)}
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "var(--gap)" }}>
          {list.map(c => <CreatorCard key={c.id} c={c} cardStyle={t.cardStyle} onClick={() => go("profile", c.id)} />)}
        </div>
      )}
    </div>
  );
...
Object.assign(window, { CreatorCard, Explore, CreatorProfile });
