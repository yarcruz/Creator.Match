...
import { PR_ZONES, PR_TOWNS } from '../data.js';

export function Brand({ name, brand }) {
  const b = name || brand || "Convoca";
  return <span className="brand-mark">{b}</span>;
}

export function Logo({ name = "Convoca", onClick, light = false }) {
  return (
    <button onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 9 }}>
      <span style={{
        ...
        <span style={{ width: 9, height: 9, borderRadius: 99, border: "2.5px solid var(--on-accent)" }} />
      </span>
...
export function Button({ children, variant = "primary", size = "md", onClick, full, type, style }) {
...
export function Chip({ children, active, onClick, accent }) {
  const [h, setH] = useState(false);
...
export function Avatar({ name, hue = 264, size = 44, ring, src }) {
...
export function PhotoUpload({ value, onChange, size = 84, name = "" }) {
...
export function Card({ children, style, hover, onClick, pad }) {
  const [h, setH] = useState(false);
...
export function inputStyleSelect() {
  return {
    width: "100%", padding: "12px 14px", borderRadius: "var(--r)",
...
export function OptionTile({ label, sub, icon, active, onClick }) {
  const [h, setH] = useState(false);
  return (
    <button onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{
...
export function LocationSelect({ value, onChange, includeAll, allLabel = "Toda ubicación", includeIsla, style: customStyle }) {
  return (
    <select value={value} onChange={e => onChange && onChange(e.target.value)} style={customStyle || { ...inputStyleSelect() }}>
