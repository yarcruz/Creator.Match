...
import { CREATORS, EVENTS, PROPOSALS, catLabel } from './data.js';
...
// false = Pro/Plans page is hidden throughout the app.
// Set to true to enable, or toggle via Tweaks → "Premium".
...
// Hydrate saved registrations into live arrays once (newest first).
(function hydrate() {
          ...
          {options.map((o, i) => (
            <button key={i} onClick={() => pick(o.screen)}
              ...
              <span style={{ width: 46, height: 46, borderRadius: 13, flexShrink: 0, display: "grid", placeItems: "center",
                background: o.dark ? "var(--ink)" : "var(--accent)", color: o.dark ? "var(--canvas)" : "var(--on-accent)",
                fontSize: 20 }}>{o.icon}</span>
              <span style={{ flex: 1 }}>
          ...
          {navItems(monetize).map(([k, lbl]) => (
            <button key={k} onClick={() => go(k)}
              style={{ padding: "8px 14px", borderRadius: 99, fontSize: 14, fontWeight: 500,
                ...
                background: route.screen === k ? "var(--canvas-2)" : "transparent" }}>
              {lbl}
            </button>
          ))}
        </nav>
...
export default function App() {
  ...
  const acceptProposal = (id) => {
    ...
    const sp = saved.find(x => x.id === id);
    if (sp) { sp.status = "aceptada"; saveLS(LS_P, saved); }
    setVersion(v => v + 1);
  };
  ...
  switch (route.screen) {
    ...
    case "dashboard":   screen = <Dashboard go={go} t={t} onAccept={acceptProposal} monetize={monetize} dashInit={dashInit} />; break;
    ...
    <div style={{ display: rail ? "flex" : "block", minHeight: "100vh" }}>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakRadio label="Tema" value={t.theme}
          options={[{ value: "editorial", label: "Editorial" }, { value: "moderno", label: "Moderno" }]}
          onChange={v => setTweak("theme", v)} />

        <TweakSection label="Marca" />
        <TweakText label="Nombre" value={t.brandName} onChange={v => setTweak("brandName", v)} />
        <TweakColor label="Acento"
          value={ACCENTS[t.accent] ? `oklch(0.55 ${ACCENTS[t.accent].c} ${ACCENTS[t.accent].h})` : ""}
