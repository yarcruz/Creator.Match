...
import {
  ...
  OptionTile, Stepper, LocationSelect, inputStyleSelect,
} from '../components/ui.jsx';
import {
  CATEGORIES, EVENT_TYPES, EVENT_STEPS, PAY_SUGGESTIONS, CREATORS,
  catLabel, modeOf,
} from '../data.js';
    ...
    <div>
      ...
      <Card pad style={{ "--pad": "24px" }}>
        ...
        <div style={{ fontSize: 13.5, color: "var(--ink-faint)", marginTop: 5 }}>
          {(producer || "Productor")} · {data.city || "Ubicación"}{data.location ? ` · ${data.location}` : ""}
        </div>
...
export function EventFlow({ go, onPublish }) {
  ...
  if (step === 3) return (
    <FlowShell steps={EVENT_STEPS} current={3} kicker="Pago" title="Pon las cartas sobre la mesa"
      ...
      <Field label="Tipo de compensación (selección múltiple)" hint="Elige todas las opciones que apliquen.">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {PAY_TYPES.map(p => (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {matches.map(c => (
              <div key={c.id} style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 14px",
                ...
                <Avatar name={c.name} hue={c.accent} size={42} src={c.photo} />
