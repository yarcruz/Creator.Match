import { Avatar, Button, Card, Chip, Stars, Badge } from '../components/ui.jsx';
import { CREATORS, EVENTS, catLabel } from '../data.js';

function Brand({ brand }) {
  return <span className="brand-mark">{brand || "Convoca"}</span>;
}

function HeroPanel({ wide, brand }) {
  const c = CREATORS[0];
  const e = EVENTS[1];
  return (
    <div style={{ position: "relative", display: "grid", gridTemplateColumns: wide ? "1fr 1fr" : "1fr", gap: 16 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: 56, alignItems: "center" }} className="hero-grid">
      <div>
        ...
        <p style={{ fontSize: 18.5, color: "var(--ink-soft)", maxWidth: 470, marginTop: 24, lineHeight: 1.55 }}>
          <Brand brand={brand} /> conecta a productores de eventos con creadores que cubren tu evento
      ...
      <HeroPanel brand={brand} />
    </div>
  );
}

function HeroCentered({ go, brand }) {
  return (
    <div style={{ textAlign: "center", maxWidth: 860, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 56 }}><HeroPanel wide brand={brand} /></div>
    </div>
  );
}
...
function HowItWorks({ brand }) {
  const steps = [
    { n: "01", t: "Muestra lo que ofreces",
      d: "Los creadores arman su perfil con portafolio, formatos que cubren y modalidad. Los productores publican el brief de cobertura del evento." },
    { n: "02", t: (<><Brand brand={brand} /> hace el match</>),
      d: "Cruzamos formato, modalidad, pago, zona y fecha — y te explicamos por qué cada nombre tiene sentido para tu evento." },
    { n: "03", t: "Cubre y publica",
      d: "El creador cubre el evento y entrega: en colaboración en ambas redes, o para tu uso exclusivo. Todo en un solo lugar." },
  ];
  return (
...
function FeaturedCreators({ go, cardStyle, monetize }) {
  return (
    <section style={{ marginTop: 110 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "var(--gap)" }} className="how-grid">
        {CREATORS.slice(0, 3).map(c => (
          <CreatorCardSimple key={c.id} c={c} cardStyle={cardStyle} monetize={monetize} onClick={() => go("profile", c.id)} />
        ))}
      </div>
    </section>
  );
...
function CreatorCardSimple({ c, onClick, cardStyle = "bordered", monetize }) {
...
export function Landing({ go, t, brand, monetize }) {
  return (
    <div className="fade-in" style={{ maxWidth: "var(--maxw)", margin: "0 auto", padding: "48px clamp(20px,4vw,40px) 0" }}>
      {t.heroLayout === "centered" ? <HeroCentered go={go} brand={brand} /> : <HeroSplit go={go} brand={brand} />}
      ...
      <FeaturedCreators go={go} cardStyle={t.cardStyle} monetize={monetize} />
      <ClosingCTA go={go} />
    </div>
  );
}
