    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 28 }} className="profile-grid">
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {MATCHES.map(m => {
            const c = creatorById(m.creator);
            const e = eventById(m.event);
            return (
              <Card key={m.id} hover onClick={() => go(role === "productor" ? "profile" : "dashboard", m.creator)}
                ...
                {role === "productor"
                  ? <Avatar name={c.name} hue={c.accent} size={48} src={c.photo} />
                  : <div style={{ width: 48, height: 48, borderRadius: 12, background: "var(--accent-wash)", display: "grid",
                      placeItems: "center", color: "var(--accent-ink)", fontSize: 20, flexShrink: 0 }}>◆</div>}
      ...
      <div>
        ...
        {role === "productor"
          ? <EventPreviewCard data={{ ...EVENTS[0] }} large />
          : <CreatorPreviewCard data={{ name: "Mariana Restrepo", headline: "Fotógrafa de cobertura",
              city: "San Juan", types: ["cobertura", "previa", "entrevista"], modes: ["colab", "exclusivo"], tier: "pro",
              tags: ["Bodas", "Corporativo", "Editorial"], rate: "$480", rateUnit: "evento",
              avail: "Disponible esta semana", bio: CREATORS[0].bio, payPref: "todos" }} large />}
      ...
      {PROPOSALS.map(p => {
        ...
        return (
          <Card key={p.id} pad style={{ "--pad": "20px", display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
            <Avatar name={c.name} hue={c.accent} size={46} src={c.photo} />
...
function MessagesTab({ go, initialCreator }) {
  ...
  const startId = initialCreator
    ? (threads.find(m => m.creator === initialCreator) || threads[0]).id
    : threads[0].id;
    ...
    <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 20, height: 520 }} className="msg-grid">
        ...
        {threads.map(m => {
          ...
          return (
            <button key={m.id} onClick={() => setActive(m.id)}
              style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 14px", borderRadius: "var(--r)",
                textAlign: "left", background: active === m.id ? "var(--accent-wash)" : "transparent",
                transition: "background 0.15s ease" }}>
              <Avatar name={mc.name} hue={mc.accent} size={42} src={mc.photo} />
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {m.unread > 0 && (
                <span style={{ background: "var(--accent)", color: "var(--on-accent)", fontSize: 11, fontWeight: 700,
                  minWidth: 19, height: 19, borderRadius: 99, display: "grid", placeItems: "center", padding: "0 5px" }}>
                  {m.unread}
                </span>
              )}
            </button>
      ...
      <Card style={{ display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "16px 20px", borderBottom: "1px solid var(--line-soft)" }}>
          <Avatar name={c.name} hue={c.accent} size={40} src={c.photo} />
          <div>
        ...
        <div style={{ padding: 16, borderTop: "1px solid var(--line-soft)", display: "flex", gap: 10 }}>
          <input placeholder="Escribe un mensaje…"
            style={{ flex: 1, height: 42, padding: "0 16px", borderRadius: 99, border: "1px solid var(--line)",
              background: "var(--canvas)", fontSize: 14, color: "var(--ink)", outline: "none" }} />
