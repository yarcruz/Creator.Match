// Content formats a creator can offer / a producer can request
...
export const PR_ZONES = ["Zona Metro", "Zona Oeste", "Zona Noreste", "Zona Suroeste", "Zona Este"];
export const PR_TOWNS = [
...
export const LOCATIONS = [...PR_ZONES, ...PR_TOWNS];
export const CITIES = LOCATIONS;

export const PAY_SUGGESTIONS = [
  ...
  { label: "$1.2k – $2.5k", note: "Full package o multi-cámara" },
];

// Mutable arrays so new registrations can be prepended and reactive via version counter
...
export const CREATOR_STEPS = ["Contacto", "Qué cubres", "Portafolio", "Preferencias", "Revisión"];
export const EVENT_STEPS   = ["Tu evento", "Fecha y lugar", "Qué necesitas", "Pago", "Publicar"];

export const CREATOR_TAGS = [
  "Bodas", "Corporativo", "Editorial", "Festivales", "Cinemático",
  "Short-form", "Bilingüe", "En vivo", "Marca", "Documental",
];

export const PAY_PREFS = [
...
export function payPrefLabel(id){ const p = PAY_PREFS.find(x => x.id === id); return p ? p.label : "Todos los eventos"; }
...
export function isBucketLink(url) {
  const u = (url || "").toLowerCase();
  if (/drive\.google\.[^/]*\/drive\/folders/.test(u)) return true;
  if (/drive\.google\.[^/]*\/(open|file).*folders/.test(u)) return true;
  if (/dropbox\.com\/(sh|scl\/fo)/.test(u)) return true;
  if (/wetransfer\.com|we\.tl/.test(u)) return true;
  if (/mega\.(nz|io)\/folder/.test(u)) return true;
  if (/photos\.app\.goo\.gl|photos\.google/.test(u)) return true;
  if (/icloud\.com\/(sharedalbum|photos)/.test(u)) return true;
  return false;
}
export function isSingleItemLink(url) {
  const u = (url || "").toLowerCase();
